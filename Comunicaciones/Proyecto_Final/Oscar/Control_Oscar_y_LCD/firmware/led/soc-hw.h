...
typedef struct {
	...
	uint32_t Tecla;     //toca definir conexion con el Wb
	uint32_t Fin_Cancion;
	uint32_t En_S_Us;
	uint32_t LEDs_ON;
	uint32_t Melodia_ON;
	uint32_t En_Mem;
...
int Cancion_Select(int Tecla);
int Nivel_Select(int Tecla);
int Led_Select(int Temp_Nivel);
int Select_OV(int Tecla);
...
void titulo2(int paso);

void song(int paso);

void song1(int paso);
void song2(int paso2);
void song3(int paso3);
void song4(int paso4);
void song5(int paso5);
 ...
 */
extern perifericos_t  *perifericos0;
